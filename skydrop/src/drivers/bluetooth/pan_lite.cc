...
CreateStdOut(btOut, bt_link.StreamWrite);
...
bool pan_light::SetBaudrate(uint32_t baud)
{
	fprintf_P(btOut, "AT+JSBR=%07lu\r\n", baud);
...
void pan_light::StreamWrite(uint8_t data)
{
//	DEBUG("%c", data);
	this->usart.Write(data);
}

bool pan_light::WaitForOK(uint16_t timeout_ms)
{
	this->ok_ans = false;
	this->error = false;
...
bool pan_light::SetName(const char * name)
{
	uint8_t nameLength = strlen_P(name);
	if (nameLength > 18)
		return false;

	fprintf_P(btOut, "AT+JSLN=%02i,%S\r\n", nameLength, name);

	return this->WaitForOK();
}

bool pan_light::SetDiscoverable(bool discoverable)
{
	fprintf_P(btOut, "AT+JDIS=%i\r\n", (discoverable) ? 3 : 0);

	return this->WaitForOK();
}

bool pan_light::CreateService(const char * uuid, const char * name, uint8_t channel, const char * deviceClass)
{
	uint8_t nameLength = strlen_P(name);
	if (nameLength > 16)
		return false;
...
bool pan_light::AcceptConnection()
{
	fprintf_P(btOut, "AT+JACR=1\r\n");

	return this->WaitForOK();
}

void pan_light::FindRN()
{
//	return;
	uint8_t c;
...
void pan_light::StreamHead(uint16_t len)
{
	bool first = true;
	while (!this->isIdle())
	{
	...
	fprintf_P(btOut, "AT+JSDA=%03d,", len);
}

void pan_light::StreamTail()
{
	fprintf_P(btOut, "\r\n");
...
bool pan_light::isIdle()
{
	return this->p_state == BT_STATE_START;
}

bool pan_light::isConnected()
{
	return this->connected;
}

void pan_light::Parse(uint8_t c)
{
	uint8_t param[2];
	uint8_t tmp;
	uint8_t n;
