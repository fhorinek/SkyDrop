...
void show_waypoint()
{
	...
	sprintf_P(message, PSTR("Next Waypoint %d/%d\n\"%s\""), fc.flight.waypoint_no, fc.flight.waypoints_count, fc.flight.next_waypoint.name);
					...
					*(start + len) = 0;
					...
					if ( end != NULL ) {
						...
						if ( end != NULL ) {
							...
							num--;
							if ( num == 0 ) break;                       // "num" allows to select a specific waypoint in the file.
						}
					}
				}
			}
