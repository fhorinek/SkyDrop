...
void imu_MadgwickQuaternionUpdate()
{
	...
	float dead_angle = degreesToRadians( 3 );
	if(abs(gx) < dead_angle)
		gx = 0;
	if(abs(gy) < dead_angle)
		gy = 0;
	if(abs(gz) < dead_angle)
		gz = 0;
...
*/
	...
	float vert_accel = imu_GravityCompensatedAccel(fc.acc.vector.x, fc.acc.vector.y, fc.acc.vector.z, fc.imu.quat );
