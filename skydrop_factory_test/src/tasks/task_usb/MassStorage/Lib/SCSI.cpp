...
static SCSI_Request_Sense_Response_t SenseData =
	{
		...
		.AdditionalSenseQualifier = 0,
		.FieldReplaceableUnitCode =	0,
		.SenseKeySpecific		= {0, 0},
...
//	DEBUG("%02X ", CommandBlock.SCSICommandData[0]);
	...
	switch (CommandBlock.SCSICommandData[0])
	{
		...
		case SCSI_CMD_READ_FORMAT_CAPACITIES:
		    CommandSuccess = SCSI_Command_Read_Format_Compacities();
		    break;
...
static bool SCSI_Command_Inquiry(void)
{
    ...
    {
        ptr = Page_Inquiry_Data;
        BytesTransferred = 5;
    }

	/* Write the INQUIRY data to the endpoint */
	Endpoint_Write_Stream_LE(ptr, BytesTransferred, NULL);
 ...
 *  \return Boolean \c true if the command completed successfully, \c false otherwise.
 */
static bool SCSI_Command_Read_Format_Compacities(void)
{
    uint8_t BytesTransferred = 12;
    Endpoint_Write_32_LE(0x00000008); // Capacities Length 8
    Endpoint_Write_32_BE(SDCardManager_GetNbBlocks());
    Endpoint_Write_32_BE(VIRTUAL_MEMORY_BLOCK_SIZE | (uint32_t)0x02 << 24);  // Descriptor code is  0x02 which means Formatted Media
