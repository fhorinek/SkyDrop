...
void gui_list_draw()
{
	...
	for (uint8_t i = 0; i < gui_list_size; i++)
	{
		...
		flags = 0;
		if (i < gui_list_size - 1 || gui_list_back == GUI_NONE)
		{
...
void gui_list_set(gui_list_gen * f_ptr, gui_list_act * f_act, uint8_t size, uint8_t back)
{
	...
	gui_list_size = size + ((back != GUI_NONE) ? 1 : 0);
	gui_list_back = back;
	gui_list_y_offset = 0;
...
{
	if (gui_list_index[gui_task] == gui_list_size - 1 && gui_list_back != GUI_NONE)
	{
...
void gui_list_irqh(uint8_t type, uint8_t * buff)
{
	...
	case(TASK_IRQ_BUTTON_M):
		if (*buff == BE_CLICK)
			gui_list_action();
		if (*buff == BE_LONG && gui_list_back != GUI_NONE)
			gui_switch_task(gui_list_back);
	break;
	}
}
