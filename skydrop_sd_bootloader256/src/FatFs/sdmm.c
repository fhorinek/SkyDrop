...
#define CS_H()	SD_SS_HI
#define CS_L()	SD_SS_LO
...
DWORD get_fattime (void)
{
	...
	return	  ((DWORD)(year - 1980) << 25)	/* Year 2013 */
			| ((DWORD)month << 21)				/* Month 7 */
			| ((DWORD)day << 16)				/* Mday 28 */
...
uint8_t spi_xmit(uint8_t data)
{
	...
	USARTF0.DATA = data;
	while ((USARTF0.STATUS & USART_TXCIF_bm) == 0);
...
//	SPIC.CTRL = (SPIC.CTRL & 0b01111100) | 0b00000011; // fsck == 250k

	USARTF0.CTRLB = USART_RXEN_bm | USART_TXEN_bm;
	USARTF0.CTRLC = USART_CMODE_MSPI_gc;
	// fsck == 250k
	USARTF0.BAUDCTRLA = 63;
	USARTF0.BAUDCTRLB = 0;
	...
	if (ty) {			        // Initialization succeded
		//SPIC.CTRL = (SPIC.CTRL & 0b01111100) | 0b10000000; // fsck == 16M
